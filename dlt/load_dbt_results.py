...
One row per dbt node result. Append-only — every run adds rows.
Idempotent: creates schema and table if missing; emits zero rows
gracefully when run_results.json doesn't exist yet (used as a bootstrap
on first ever invocation).

Departure note: docs/schema.sql describes a `bronze.raw_dbt_results`
table; this script writes to `main_bronze.raw_dbt_results_raw`. The
`main_` prefix matches dbt-duckdb's existing schema convention; the
`_raw` suffix marks this as a dlt landing table that downstream admin
dbt models read directly (no separate dbt-managed bronze view).
"""
...
def parse_run_results() -> list[tuple]:
    ...
    for r in data.get("results", []):
        ...
        # node_id formats:
        #   test.<package>.<readable_test_name>.<random_hash>
        #   model.<package>.<model_name>
        # For tests we want the readable name (3rd segment), not the
        # last segment which is dbt's autogenerated hash.
        parts = node_id.split(".")
...
def main() -> int:
    ...
    try:
        ...
        print(f"  destination: {DUCKDB_PATH} :: {SCHEMA}.{TABLE}")
        return 0
    finally:
        con.close()
