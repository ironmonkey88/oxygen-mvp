...
| 2026-05-10 10:16 ET | Worktree-scoped settings.local.json writes allowed in committed settings.json | Plan 5 D1 hotfix (commit `b78d3d5`) — adds explicit `Read(.claude/worktrees/*/.claude/settings.local.json)` + `Write(...)` patterns. Lands as a forward-looking pattern fix on top of the closed Plan 5 D1 (commit `b274ae7`); future Code sessions can re-empty the local file without prompting Gordon. |
| 2026-05-10 10:35 ET | Agent prompt: never state a calendar year in prose without querying it | Plan 6 follow-on (commit `3603136`). Q2 test bench exposed Sonnet 4-6 hallucinating "2025" when SQL evaluated `year(current_date)` to 2026. New `system_instructions` hard rule: if SQL uses `current_date` and prose needs the year, run `SELECT year(current_date)` first. Verified via Q2 retest. |
| 2026-05-10 10:55 ET | block-code-padded sentinel form corrected: "NA"-space-padded, not whitespace | Plan 8 follow-on (commit `3ea828e`). Q4 test bench showed top block was `"NA             "` (NA + 13 spaces, 38% of rows). Original entry's workaround (`trim(block_code) = ''`) never matched in real data. Rewritten with verified shape + correct workaround SQL. Validates the broader lesson: limitation entries sourced from briefs need to be sanity-checked against the warehouse. |
| 2026-05-10 11:05 ET | dept-tags-as-booleans coverage corrected: 987 / 0.08%, not 4,187 / 0.36% | Plan 8 follow-on (commit `1877ee0`). Audited via `scratch/audit_limitation_claims.py`. Brief's number derived from the 2026-05-07 LOG decision row, which appears to have been an early-session estimate or different counting method. Other quantitative claims audited at the same time and found accurate: 2024-survey-columns-sparse (<1% verified at 0.53–0.64%), open-status-not-just-open (4 statuses present). |
