...
| 2026-05-11 | **MVP 1.5: Public chat at `/chat` via nginx Basic Auth** | Same-site experience — portal at `/` (unauth), chat SPA at `/chat` (auth-gated). nginx config (`nginx/somerville.conf`) adds `/chat`, `/assets`, `/api`, `/favicon-logo.svg` locations all proxying `localhost:3000` with `auth_basic`. `/chat` and `/api` include WebSocket upgrade headers for streaming agent replies. Credential: `/etc/nginx/.htpasswd` with `analyst` user, bcrypt hash, root:www-data 640, NOT in repo. `.gitignore` hardened (`.htpasswd`, `*.htpasswd`). Portal hero pill flipped from `<span>` to `<a href="/chat">Private beta — try the chat →</a>`. curl gates 1-3 all passed: 401 without creds, 200 with creds, portal routes unaffected, wrong creds rejected. Public `:3000` confirmed closed at AWS SG; Tailnet `:3000` still accessible to the project team. Tradeoff documented in `docs/limitations/chat-auth-basic-cleartext.md` (HTTP cleartext + single shared cred — replaced by MVP 4's Magic Link + HTTPS). See `docs/plans/mvp-1.5-public-chat-via-nginx-basic-auth.md`. |
