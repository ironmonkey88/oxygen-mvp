...
- **Public chat access (MVP 1.5).** The Oxygen SPA at port 3000 is publicly proxied through nginx at `/chat`, gated by nginx Basic Auth (`/etc/nginx/.htpasswd`, single shared `analyst` credential, bcrypt hash, NOT in repo). SPA asset paths required for the SPA to load (`/assets`, `/api`, `/favicon-logo.svg`) are also auth-gated and proxied to `localhost:3000`. WebSocket upgrade headers passed through `/chat` and `/api` for streaming agent replies. Portal routes (`/`, `/docs/`, `/metrics`, `/trust`) remain unauthenticated as the public window. Tailnet `:3000` direct access remains open over Tailscale. This is deliberately scaffolded — HTTP cleartext, no TLS, single credential — and gets replaced in MVP 4 by Magic Link auth + HTTPS via Oxygen multi-workspace mode. See `docs/limitations/chat-auth-basic-cleartext.md` for the tradeoff statement.
