...
| variables            | Variable schema definition using JSON Schema                                              | Optional |

## Variables

Agents can define typed variables with JSON Schema validation. This enables:

* **Type Safety**: Validate input types (string, integer, boolean, etc.)
* **Documentation**: Provide clear descriptions of expected inputs
* **Defaults**: Set default values for optional parameters
* **Constraints**: Define enums, min/max values, and patterns

### Defining Variables

```yaml my-agent.agent.yml theme={null}
...
system_instructions: |
  ...
  {% if verbose %}Include detailed explanations.{% endif %}
```

### Passing Variables from Workflows

When using an agent in a workflow, you can pass variables to override defaults:

```yaml workflow.yml theme={null}
variables:
  target_env:
    type: string
    default: staging
...
tasks:
  - name: analyze_data
    ...
    prompt: Analyze recent sales trends
    variables:
      database: "{{ target_env }}"
      max_results: 500
      verbose: true
...
Variables can reference global semantics:
...
system_instructions: |
  Query data from the {{ orders_table }} table.
```
...
| src   | Path to the source file relative to the project root (can be single file or list) | "data/acme.schema.yml" |
...
```yaml semantic_model.agent.yml theme={null}
model: "openai-4.1-mini"
context:
  - name: anon_youtube
    type: semantic_model
    src: data/acme.schema.yml
system_instructions: |
  ## Instructions
