...
<Note>
  Oxy also supports a more structured semantic layer approach using views and
  topics. See the [Semantic Layer
  documentation](/learn-about-oxy/semantic-layer) for an alternative way to
  organize your semantic definitions.
</Note>

### Organization and Formatting Rules

* A semantic model should be organized under the `data` folder.
* Use the `.schema.yml` extension to distinguish it from other files in the `data` folder. e.g. `finance.schema.yml`
...
```yaml data/anon_youtube.schema.yml theme={null}
...
context:
  - name: anon_youtube_model
    type: semantic_model
    src: data/anon_youtube.schema.yml
```
...
<CardGroup cols={2}>
  <Card title="Semantic Layer" icon="layer-group" href="/learn-about-oxy/semantic-layer">
    Alternative semantic layer with views and topics
  </Card>

  <Card title="Agents" icon="robot" href="/learn-about-oxy/agents">
    Using semantic models with agents
  </Card>
