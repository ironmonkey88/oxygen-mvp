...
> Build a comprehensive semantic layer to power AI-driven analytics
...
<Note>
  Oxy also supports a simpler semantic model format (`.schema.yml`) for
  straightforward use cases. See [Semantic
  Model](/learn-about-oxy/semantic-model) for a lighter-weight alternative.
</Note>
...
```yaml semantics/views/orders.view.yml theme={null}
name: orders
...
```yaml agents/analyst.agent.yml theme={null}
