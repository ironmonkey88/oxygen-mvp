...
> How to use your semantic layer in agents, workflows, and routing agents
...
```yaml agents/analytics.agent.yml theme={null}
...
<Note>
  When querying a topic with **default\_filters**, those filters are
...
```yaml agents/multi_domain.agent.yml theme={null}
model: "openai-4o-mini"
...
Apply filters to narrow down your results. These filters are combined with any **default\_filters** defined in the topic using AND logic:
...
```yaml workflows/customer_analysis.workflow.yml theme={null}
...
```yaml agents/_data_router.agent.yml theme={null}
model: "openai-4o-mini"
type: routing
...
* Provide clear routing instructions in system\_instructions
...
* Use default\_filters in topics for common business rules
