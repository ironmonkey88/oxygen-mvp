...
> Configure and use various container runtimes with Oxy

## Overview

Oxy requires a **Docker API-compatible endpoint** to manage PostgreSQL and other containerized services. This means any container runtime that exposes a Docker-compatible API socket.
...
<Info>
  The applications listed below (Docker Desktop, Rancher Desktop, etc.) are just
  **convenient ways to install and manage** the underlying container engine. You
  can also install Docker Engine directly without any desktop application - see
  the "Docker Engine (Standalone)" tab.
</Info>
...
<Tabs>
  ...
  <Tab title="Rancher Desktop">
    ...
    <Steps>
      ...
      <Step title="Windows (WSL)">
        See the [prerequisites guide](/prerequisites#windows-wsl) for WSL 2 setup and Rancher Desktop integration.
      </Step>
...
|                 | `unix:///mnt/wsl/rancher-desktop/run/docker.sock` (WSL) |
...
<AccordionGroup>
  <Accordion title="Failed to connect to container runtime">
    ...
    **Common solutions:**
    ...
    4. Check DOCKER\_HOST environment variable:
       ```bash theme={null}
       echo $DOCKER_HOST
       ```
  </Accordion>
  ...
  <Accordion title="Rancher Desktop: daemon not responding">
    1. Open Rancher Desktop preferences 2. Switch Container Engine to **dockerd
       (moby)** 3. Restart Rancher Desktop 4. Wait for full initialization (30-60
       seconds) 5. Retry: `oxy start`
  </Accordion>

  <Accordion title="Rancher Desktop on Windows (WSL): docker command not found or socket missing">
    This is almost always caused by WSL 1 or missing WSL integration. See the [prerequisites guide](/prerequisites#windows-wsl) for WSL 2 setup steps.

    If Docker socket is not at the default path, set `DOCKER_HOST`:
...
<Info>
  On **Linux**, all runtimes perform similarly as they use native kernel
  features.
</Info>

<Info>
  On **Windows**, Docker Desktop offers the best support and performance with
  WSL2 integration.
</Info>
