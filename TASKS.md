...
**Plan 51 done 2026-06-17** — [x] session-starter.md new-account cold-start block (both repos). Added one conditional section near the top of `session-starter.md` orienting a fresh Claude in a **new account / new machine**: names the migration trio in read-order (**`docs/MIGRATION_SUMMARY.md` → `MIGRATION_CHECKLIST.md` → `PROJECT_MIGRATION_2026-06-07.md`**) then hands off to the normal "How to Start Each Session" flow; a continuing session skips it. **Consolidation, not a new doc.** Block worded identically in both repos (extracted-block `diff` clean); rest of each file repo-specific and untouched (the two are intentionally not byte-identical overall). All three migration docs verified present + git-tracked at their named paths in both repos before wiring (halt clear). This repo **Plan 51** / sibling **stack-in-a-box Plan 11**. Supersedes the earlier infra-install "zero-to-live setup runbook" prompt per Gordon's consolidation decision. Boot-audit: 41/42/45 reserved, 47 = open PR #76 → 51 next free. Prompt+report at `docs/prompts/plan-51-session-starter-coldstart.{md,report.md}`.

**Plan 50 done 2026-06-17** — [x] Project migration summary in both repos. New `docs/MIGRATION_SUMMARY.md` — a cold-start handoff so a fresh Claude Chat in a different account can resume the project with nothing lost (working agreements, in-flight state, deferred decisions + reasoning, repo locations + stack, first-session checklist). Sits at the same altitude as APPROACH.md: **byte-identical body in both repos** (`diff` clean, `shasum` match `e44f3c1…`) — this repo **Plan 50** / sibling **stack-in-a-box Plan 8**. Canonical body transcribed verbatim from the prompt (point-in-time snapshot dated 2026-06-17; the forward-looking "verify against LOG.md" lines aimed at the next Chat were transcribed as-is, not resolved). Wiring: `session-starter.md` "Key Files to Know" gains a one-line pointer framed as the new-account/fresh-context restart doc. No existing authority doc edited beyond the two session-starter pointers. Documentation-only (no live gates). Plan slot resolved by boot-audit: 41/42/45 reserved, 47 = open PR #76 → 50 is the next contiguous free slot (no ambiguity → halt did not fire). Prompt+report at `docs/prompts/plan-50-migration-summary.{md,report.md}`.

**Plan 49 done 2026-06-10** — [x] PHILOSOPHY.md creed reconciliation (Prompt A). Content-only reconciliation of `PHILOSOPHY.md` to APPROACH.md (Plan 48) — closes the "PHILOSOPHY.md generalization + the creed" follow-up the Plan 48 row flagged. §3 now names the **empathy/honesty/optimism** creed as the distillation of its three existing moves (honesty → 1, optimism → 2, empathy → 3), same creed the standard carries; **Somerville one-liner preserved**. "The resident is the measure" framed as the Somerville form of the standard's general "the people the work is about are the measure"; §6.6 back-references §3. **Untouched** (two-hunk diff, §3 + §6.6): system humanism §2.3/§5, §4, other §6 principles — the system-humanism split stays deferred until oxygen-mvp has a local METHODOLOGY.md. No contradiction (halt did not fire). Boot-audited the slot first (`gh pr list --state all`) → 49 free. Prompt+report at `docs/prompts/plan-49-philosophy-creed-reconcile.{md,report.md}`.

**Plan 48 done 2026-06-10** — [x] APPROACH.md cross-repo reference standard. (**Renumbered 47 → 48** — first merged as Plan 47 (PR #78), collided with the older open PR #76 "tech + test debt assessment" which had the prior claim on 47; per Gordon this work yielded. Branch `claude/plan-47-approach-reference-standard` + PR #78 title are historical.) New `APPROACH.md` at repo root (plain-language "how we build": empathy/honesty/optimism, trust contract, hypothesis-then-result, decide-then-build, declarative/reconciliation), tool/dataset-agnostic, **byte-identical in both repos** (`diff` clean). This repo **Plan 48**; sibling **stack-in-a-box Plan 5** (Plan 4 there reserved for the Oxygen version pin). Sits above PHILOSOPHY.md (the Somerville instance that specializes it). Wiring: CLAUDE.md gains a "Reference standard (cross-repo, above the convictions)" tier with the reconciliation rule (principle → APPROACH wins; how-it's-done → operational doc wins); session-starter.md gains a parallel bullet. Canonical body delivered separately + an aligned Google Doc; Gordon approved a **merge** (pasted body's reconciliation closer + the Doc's newcomer intro line; title kept per Phase 0). Both PHILOSOPHY.md files read in full → **compatible** (no contradiction; halt did not fire). Prompt+report at `docs/prompts/plan-48-approach-reference-standard.{md,report.md}`. Out of scope (future prompts): PHILOSOPHY.md generalization + the creed + Sensemake/system-humanism methodology.
