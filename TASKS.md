...
**Plan 34 reserved 2026-05-23** — [ ] dbt-docs back-link styled-pill fix v2 — now scopable with Plan 33's finding. Library is **marked.js** with `marked.setOptions({gfm: true, sanitize: true})`; `sanitize: true` escapes all HTML regardless of wrapping, so no Markdown-source change can produce the styled pill. Four named fix paths from Plan 33's finding (`docs/design-reviews/dbt-docs-library-inspection-2026-05-22/finding.md`): (1) drop styled-pill, use plain Markdown link `[← Back to Somerville Analytics portal](/)` and accept dbt-docs default link style; (2) nginx `sub_filter` to inject a `<style>` block targeting the first anchor in `.model-markdown`; (3) sed-replace post-`dbt docs generate` to rewrite the escaped back-link text back to HTML before serving; (4) drop the in-`/docs/` back-link entirely, surface "back" navigation at the nginx layer (header bar). Slot reserved; Gordon to write the prompt picking among the four.
