...
**Plan 40 reserved 2026-05-23** — [ ] Track D from Plan 39 prompt: DBA Dashboard v1.1. D1 — cost panel reading from `main_admin.fct_chat_activity_raw` (month-to-date + 30d sparkline + burn-rate vs last month). D2 — source-health expansion from 1 source to 6 (5 new timers/services or one parameterized). D3 — generator updates (cost panel + `data-panel-id` attributes + Group C layout restoration). D4 — design doc `docs/dba-dashboard-design-2026-05-17.md` revision (§0 noting cost-panel reversal, §3 Group C restore C2, §11 update). D5 — Playwright verification via Plan 39's `targets_selector='[data-panel-id]'`. Plus new limitations entry `cost-panel-pricing-assumptions.md`.

**Plan 41 reserved 2026-05-23** — [ ] Memory-to-file migrations per Plan 36 §3 (was Plan 38, then Plan 40, now Plan 41 after Plan 38's actual content was DBA dashboard + Plan 39's Track D got bumped to Plan 40). Five candidates: `feedback_session_boot_audit.md` → CLAUDE.md; `feedback_git_ssh_gotchas.md` → CLAUDE.md "Known gotchas"; `feedback_no_ssh_heredocs.md` → CLAUDE.md "Bash Safety"; `feedback_chat_code_handoff.md` → PROMPTS.md; `feedback_settings_json.md` → CLAUDE.md "Allowlist policy" sub-bullet. Each needs deliberate placement + wording decisions.
