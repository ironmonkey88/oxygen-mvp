...
**Plan 47 in progress 2026-06-02** — [~] Tech & Test debt assessment (INVESTIGATE & REPORT — zero behavior changes). Branch `claude/plan-47-techdebt-assessment`; prompt at `docs/prompts/plan-47-techdebt-assessment.md`. Deliverable: `docs/design/TECHDEBT_ASSESSMENT.md` with measured (not assumed) findings — page-gen DRY-vs-WET dedup proposal, Python test/CI gap (tied to methodology R1), proposed pinned `requirements.txt`, and a data-layer coherence verdict (dbt graph + dlt pipelines: team-effort vs parallel-play) + correctly-sequenced reduction plan (deps/tests before any refactor). Two pre-flight gaps resolved with Gordon: (1) EC2 read-only measurement authorized (`pip freeze` ground-truth pins; `dbt parse`/`manifest.json`/`run_results.json` for the graph verdict); (2) R1 reported in the assessment + recommended as human-ratified follow-up (don't auto-instantiate `METHODOLOGY.md`). Baseline confirmed: 31 Python files / 7,271 canonical LOC; no `requirements.txt`/`pyproject`/CI; zero Python test files.
