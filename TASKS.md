...
**Plan 35 in progress 2026-05-23** — [~] Bottom back-link consistency fix. Apply the same plain-Markdown shape Plan 34 used for the top link to the bottom-of-page link in `dbt/models/overview.md` (currently rendering as literal text per Plan 34's `back-link-dom.json`). Single edit, verify with Plan 33's helper (DOM + visual distinction + consistency with top + navigation), commit. Boot-audit first ran with new B1 step-4-extension and caught the carry-forward portal/index.html drift; resolved via separate hygiene PR ahead of Plan 35.

**Plan 36 reserved 2026-05-23** — [ ] allowlist audit (was Plan 35 before re-numbering). Scope: review `.claude/settings.json` for patterns that grant arbitrary code execution (`Bash(python *)`, `Bash(python3 *)`, `Bash(bash *)`, etc.) and decide which to tighten or remove. Flagged in Session 57's `/fewer-permission-prompts` skill run.
