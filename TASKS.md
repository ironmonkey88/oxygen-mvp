...
**Plan 34 in progress 2026-05-23** — [~] dbt-docs back-link fix v2 + carry-over drift items. Track A: try Approach #1 (plain Markdown `[← Back to Somerville Analytics portal](/)` for top link, replacing the bare `<a>` source that marked.js was escaping); verify via Plan 33's helper against `http://18.224.151.49/docs/` for DOM-is-`<a>` + visually-distinct + navigates-to-`/`. Fall through to Approach #4 (nginx-layer nav) only on explicit Chat go-ahead after #1 halts. Bottom back-link stays as-is per scoping. Track B: B1 portal/index.html regen drift (leaning option 2 — boot-audit checklist entry per Code's reasoning + memory steer); B2 `.oxy_state/apps/` inspection (likely gitignore); B3 git `http.postBuffer` 524288000 note in CLAUDE.md.

**Plan 35 reserved 2026-05-23** — [ ] allowlist audit. Slot reserved (queued in Chat). Scope: review `.claude/settings.json` for patterns that grant arbitrary code execution (`Bash(python *)`, `Bash(python3 *)`, `Bash(bash *)`, etc.) and decide which to tighten or remove. Flagged in Session 57's `/fewer-permission-prompts` skill run.
