...
**Plan 37 reserved 2026-05-23** — [ ] Two `gh` allowlist additions per Plan 36 recommendations: `Bash(gh pr create *)` and `Bash(gh pr merge *)`. Tracked-policy mismatch (CLAUDE.md autonomous-execution policy endorses them; settings.json makes Code prompt). One-line edits, depends on Plan 36 findings.

**Plan 38 reserved 2026-05-23** — [ ] Memory-to-file migrations per Plan 36 §3. Five candidates: `feedback_session_boot_audit.md` → CLAUDE.md "Session Boot Audit" subsection; `feedback_git_ssh_gotchas.md` → CLAUDE.md "Known gotchas" extension; `feedback_no_ssh_heredocs.md` → CLAUDE.md "Bash Safety"; `feedback_chat_code_handoff.md` → PROMPTS.md §5/§6; `feedback_settings_json.md` → CLAUDE.md "Allowlist policy" sub-bullet. Each is a real doc edit; needs deliberate placement + wording decisions, not bolt-on.
